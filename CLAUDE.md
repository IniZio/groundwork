...
**Human read path:** The charter (`motive.md`) and ticket corpus (`tickets/`) under `.groundwork/motives/<slug>/` are the authoritative read path for humans reviewing progress. Derived projections (`MAP.md`, `open-items/`, `TRACE.html`) are prohibited (D-24): they regenerate from a corpus and can silently disagree with it. The SessionStart injection does not enumerate any generated index files.

**Ticket-first model:** Tickets are hand/agent-authored documents stored under `.groundwork/motives/<slug>/tickets/`. They are **never auto-generated per ledger slice** and **never deleted or overwritten by regeneration** — tooling only creates a ticket file when it does not already exist. The one sanctioned exception is `journal migrate-tickets <slug>`, which deletes only legacy auto-generated tickets identified by a footer on their last non-empty line. Use `tickets/` for work objects that must survive across sessions and regeneration cycles.

**Ledger CLI command reference** (use these; MUST NOT Read/Edit the run ledger file — `.groundwork/runs/<session_id>.json`, legacy `.groundwork/run.json` — by hand). Every `gw ledger` subcommand requires `--motive <slug>` matching the ledger's recorded motive. **`gw ledger init` is the documented default** — ledger initialization: `gw ledger init <file|-> --motive <slug>` (reads initial ledger JSON from a file or stdin; pass `--token <write_token>` to overwrite a tokened active run, or `--force` to overwrite a tokenless active run). Invoke `gw` via `bin/gw-hook` symlinked to PATH (e.g. `ln -s $(pwd)/bin/gw-hook ~/.local/bin/gw`; no `bin/gw` wrapper exists):
...
Reject an advisor verdict whose test evidence came from a filtered run (a named file list rather than the whole suite), or whose exit code passed through a pipe. Reject also any verdict whose suite run was not environmentally controlled. The gate suite MUST run in the real working tree — never a detached worktree — from the repo root, with the harness's ambient env scrubbed: a plugin-hosted session inherits `CLAUDE_PLUGIN_ROOT` (pointing at whichever plugin's hook ran last, so schema loads ENOENT against a foreign plugin cache) and `CLAUDE_PROJECT_DIR` (so `gw` parity tests read the live repo ledger instead of their temp fixture, surfacing as `MOTIVE_MISMATCH`). `CLAUDE_CODE_SESSION_ID` MUST be set, not scrubbed — ledger tests fail without it. The only admissible gate invocation is:
`env -u CLAUDE_PROJECT_DIR -u CLAUDE_PLUGIN_ROOT CLAUDE_CODE_SESSION_ID=gate npx vitest run`
Evidence rules live in the advisor's Verification Protocol; the orchestrator does not run them, it refuses verdicts that skipped them.
...
**Acceptance criteria are withdrawn, never rewritten.** A criterion is a derived artifact, transcribed from a decision, charter objective, or spec, and the transcription can be wrong. Correcting a mis-transcription against its source is legitimate before the slice is closed, and the corrected text MUST cite the source id. Editing the `acceptance` text of the slice you are closing so the delivered result satisfies it is not — no gate reviews orchestrator-authored criterion text, so disclosure is the only control. When measurement disproves a criterion's premise, retract the coverage claim with `journal ac-retract --motive <slug> --ac <id> --slice <id> --reason <measurement>` and record a DECISION naming the falsifying measurement; a surviving obligation moves to a successor slice under the `ledger add` rule above and is named in the retraction. In a criteria table a criterion reads `MET` or `WITHDRAWN` — an investigation outcome ("premise falsified") is not a status. Test: would you accept this edit from a subagent closing its own slice?
...
- **Runtime:** Node.js v22+ with native ESM (`"type": "module"`). **bun is required for hook enforcement.** Seven hooks (agent-model-guard, nesting-guard, ledger-guard, ledger-bash-guard, piped-exit-code-guard, orchestrator-impl-guard, stop-gate) run via `bin/gw-hook`, which requires bun. No node fallback: node cannot remap the NodeNext `.js` import specifiers in `src/gw/**` to `.ts`, and `dist/gw.mjs` is a `--target=bun` bundle — both routes crash. Resolution order: `$GW_BUN` → PATH → `~/.bun/bin`, mise shims/installs, `~/.local/bin`, `/usr/local/bin`, `/opt/homebrew/bin` (covers non-login PATH launchers). With no bun anywhere, all 7 hooks fail loudly (non-zero exit, stderr diagnostic naming bun and the searched locations). The remaining 6 hook registrations plus session-reminder are plain `.mjs` files and do not require bun. Hook invocations: ~65–93ms (vs. ~15–30ms for `.mjs` hooks — 2–3× slower). TypeScript in `src/` compiled with `tsc`. Bun is also used for the test suite.
