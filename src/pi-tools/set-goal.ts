...
export function createSetGoalTool(deps: ToolDeps) {
  ...
  return defineTool({
    ...
    parameters: Type.Object({
      action: Type.String({
        description: 'Action to perform: "set", "status", "pause", "resume", "achieved", or "clear".',
      }),
      ...
      acceptanceCriteria: Type.Optional(
        Type.Array(Type.String(), {
          description: 'List of verifiable acceptance criteria (required for "set" action).',
        }),
      ),
    }),
    async execute(
      ...
      const acceptanceCriteria = params.acceptanceCriteria as string[] | undefined;
      const sessionID = (ctx as any)?.sessionManager?.getSessionId?.() ?? "";

      if (!sessionID) {
        return {
          content: [
            { type: "text", text: "Error: No session ID available. Cannot manage goal." },
          ],
          details: undefined,
        };
      }
      ...
      switch (action) {
        ...
        case "resume": {
          ...
          if (goal.status !== "paused") {
            return {
              content: [{ type: "text", text: `Goal is ${goal.status}, not paused.` }],
              details: undefined,
            };
          }
          goal.status = "active";
          writeGoal(directory, sessionID, goal);
          return {
            content: [{ type: "text", text: `Goal resumed: "${goal.objective}"` }],
            details: undefined,
          };
        }
        ...
        case "clear": {
          ...
          return {
            content: [{ type: "text", text: removed ? "Goal cleared." : "No goal to clear." }],
            details: undefined,
          };
        }
