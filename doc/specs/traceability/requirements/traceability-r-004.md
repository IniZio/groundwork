...
The traceability graph assembler **shall** be a pure function of its input data: given the same ledger slices, journal events, spec requirements, and .groundwork/spec-build/coverage.json, it **shall** produce byte-for-byte identical node and edge sets on every invocation, with no timestamps, random ids, or nondeterministic ordering.
...
- **Fit criterion** — Running the assembler twice against the same frozen inputs (ledger snapshot, journal snapshot, .groundwork/spec-build/coverage.json snapshot) produces identical JSON output both times; diffing the two outputs reports no differences.
- **Verification**: automated — Automated test asserting deterministic output:
  1. Freeze a set of inputs: ledger slice list, journal event list, spec requirements, .groundwork/spec-build/coverage.json.
