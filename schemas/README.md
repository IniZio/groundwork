...
```json
{ "$id": "run-ledger.schema.json", "$schema": "https://json-schema.org/draft/2020-12/schema" }
```
...
if (!validate(data)) {
  ...
  const lines = ajvErrorsToLines(validate.errors, 'ledger')
  for (const line of lines) process.stderr.write(`ledger: validate: ${line}\n`)
  process.exit(1)
}
```

`ajvErrorsToLines` output matches the existing `<field>: <problem>` format
emitted by `hooks/spec-lint.mjs`. No changes to those runners' output format
are needed.
