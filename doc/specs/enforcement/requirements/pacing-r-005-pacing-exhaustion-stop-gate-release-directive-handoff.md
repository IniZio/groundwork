...
- **Fit criterion** — With pacing exhausted and two incomplete slices remaining, the Stop hook exits 0 (session is permitted to end) and its output contains a directive line naming: the motive slug and both incomplete slice ids. With pacing active (budget not exhausted) and incomplete slices remaining, the Stop hook still blocks as before (unchanged behaviour).
- **Verification**: automated — with pacing exhausted and two incomplete slices, confirm Stop hook exits 0 and its output includes a directive naming the motive slug and both slice ids; with budget remaining, confirm Stop hook still blocks on incomplete slices.
- **Criticality**: must
