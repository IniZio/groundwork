...
export const GROUNDWORK_VERSION = "3.7.0";
...
**Criterion: comment-density** — run \`bin/gw-hook comment-density report --json\` over the session's touched files and list every file reported in \`data.files\` (each is over-cap or carries flagged comments). APPROVE is blocked while any flagged file has no registered cleanup slice in the active ledger. To clear: run \`bin/gw-hook comment-density report --json | bin/gw-hook comment-density remediate-plan --motive <slug>\` and dispatch one haiku cleanup slice per flagged file, all in one wave.
...
3. **Ledger CLI only.** Never Read/Edit \`.groundwork/run.json\` directly. Use the ledger CLI for all run ledger mutations (complete, set, add, rm, gate, abandon). Commands are injected by the SessionStart hook's "Groundwork CLI tools" block; manual form: \`gw ledger --motive <slug> <subcommand>\` (\`gw\` = \`bin/gw-hook\` symlinked to PATH; \`gw ledger init <file|-> --motive <slug>\` initializes a new run from a JSON file or stdin).
