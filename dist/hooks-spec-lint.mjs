#!/usr/bin/env node
// @bundle-source-hash: 0e9e8b990724eda3601ad1c1ce2c60d2f1f231a78dc2e3b8c85528bf84c9a383
// @bun
...
// hooks/spec-lint.mjs
import { readFileSync as readFileSync4, existsSync as existsSync2, readdirSync as readdirSync3, statSync as statSync2 } from "fs";
import { join as join3, dirname as dirname3, basename as basename2, resolve as resolve2, relative as relative3 } from "path";
...
var ajv = new import__2020.default({ strict: true, allErrors: true });
...
var ALLOWED_FRONTMATTER_FIELDS = new Set([
  "id",
  "type",
  "concept",
  "parent",
...
function firstSentence(text) {
  const clean = (text || "").replace(/^#+\s.*$/mg, "").replace(/[`*_[\]]/g, "").trim();
  const m = clean.match(/[^.!?]*[.!?]/);
  return m ? m[0].trim() : clean.slice(0, 120).trim();
}
var ID_RE_SRC = "\\b([A-Z][A-Z0-9]*(?:-[A-Z0-9]+)*-R-[a-z0-9]+|C-[A-Z][A-Z0-9]*(?:-[A-Z0-9]+)*)\\b";
...
function pathLikeToken(text) {
  ...
  for (const t of tokens) {
    ...
    if (t.includes("/"))
      return t;
    if (/\.[a-zA-Z]{1,5}$/.test(t) && t.length > 4 && !t.endsWith("etc."))
      return t;
  }
  return null;
}
...
function findNearestConceptId(reqAbsPath, sd) {
  const sdNorm = sd.replace(/\/?$/, "");
  let dir = dirname2(reqAbsPath);
  for (let i2 = 0;i2 < 12; i2++) {
    if (dir === sdNorm || dirname2(dir) === dir)
      break;
    for (const filename of ["index.md", "README.md"]) {
      const candidate = join(dir, filename);
      if (existsSync(candidate)) {
        const { data } = parseYamlFrontmatter(readFileSync2(candidate, "utf8"));
        if (data.id)
...
function resolveConceptRef(rawConcept, sd) {
  ...
  if (existsSync(indexPath)) {
    const { data } = parseYamlFrontmatter(readFileSync2(indexPath, "utf8"));
    if (data.id)
      return String(data.id);
  }
  return s;
...
function buildIndexData(sd) {
  ...
  for (const { absPath, relPath } of files) {
    ...
    if (viewFilePaths.has(absPath) && _bn !== "README.md" && _bn !== "constraints.md" && _bn !== "requirements.md") {
      let _viewRaw;
      try {
        _viewRaw = readFileSync2(absPath, "utf8");
      } catch {}
      if (_viewRaw && /^##+ [A-Z].*-R-\d/m.test(_viewRaw)) {
        warnings.push({
          type: "view_shadows_requirements",
          path: absPath,
    ...
    if (relPath.split("/").some((p) => p === "design" || p === "decisions"))
      continue;
    if (_bn === "glossary.md")
      continue;
    let raw;
    ...
    if (filename === "constraints.md" || filename === "requirements.md") {
      ...
      for (const section of sections) {
        ...
        const summary = section.normativeStatement ? firstSentence(section.normativeStatement) : section.title;
        nodes[id] = {
          id,
          type: "requirement",
          title: section.title,
...
`, headingMatch.index) + 1).trim();
      ...
      const summary = data.summary ? String(data.summary) : effectiveEars ? firstSentence(effectiveEars) : h1 ?? (data.title ? String(data.title) : firstSentence(body || id));
      nodes[id] = {
        id,
      ...
      if (basename(absPath) === "README.md") {
        const dirViews = conceptDirViews.get(dirname2(absPath));
        if (dirViews)
          nodes[id].views = dirViews;
      }
    }
  }
...
function githubSlug(text) {
  return text.toLowerCase().replace(/[^\p{L}\p{N}\s-]/gu, "").trim().replace(/[\s_]+/g, "-").replace(/-+/g, "-").replace(/^-+|-+$/g, "");
}
function extractAllHeadingAnchors(markdown) {
  const { body } = parseYamlFrontmatter(markdown);
  const anchors = new Set;
  const bodyWithoutFences = body.replace(/^( {0,3})(```+|~~~+)[^\n]*\n[\s\S]*?^\1\2[ \t]*$/gm, "");
...
var IGNORED_DIRS = new Set(["node_modules", "worktrees"]);
var TEST_DIRS = ["test", "tests"];
var TEST_EXTS = new Set([".ts", ".js", ".mjs", ".mts"]);
...
var VERIFIES_COMMENT_RE = /^\s*(?:\/\/|\*)\s*@verifies\b/;
function extractVerifiesFromLine(line) {
  if (!VERIFIES_COMMENT_RE.test(line))
    return [];
...
function scanVerifies(rootDir) {
  const result = new Map;
  for (const testDirName of TEST_DIRS) {
    const dir = join2(rootDir, testDirName);
...
function lookupVerifies(verifiesMap, reqId) {
  return verifiesMap[reqId.toLowerCase()] ?? [];
}

// hooks/spec-lint.mjs
function loadSpecIndex(projectDir) {
  const specDir = join3(projectDir, "doc", "specs");
...
function loadSpecManifestSync(conceptDir) {
  const p = join3(conceptDir, "spec.yaml");
  if (!existsSync2(p))
    return { manifest: null, errors: [] };
  let raw;
  try {
  ...
  if (valid)
    return { manifest, errors: [] };
  const errors = (validate.errors || []).map((err) => ({
    field: err.keyword === "required" ? err.params?.missingProperty || "(root)" : err.instancePath ? err.instancePath.replace(/^\//, "") : "(root)",
    problem: err.message || "invalid"
  }));
  return { manifest, errors };
}
function findRfcDirSync(projectDir, uid) {
  const rfcsDir = join3(projectDir, ".groundwork", "rfcs");
  if (!existsSync2(rfcsDir))
    return null;
  let entries;
  try {
    entries = readdirSync3(rfcsDir);
  } catch {
    return null;
  }
  for (const name of entries) {
    const dir = join3(rfcsDir, name);
    try {
    ...
    const yamlPath = join3(dir, "rfc.yaml");
    if (existsSync2(yamlPath)) {
      try {
        const parsed = load(readFileSync4(yamlPath, "utf8"));
        if (parsed && parsed.uid === uid)
          return dir;
      } catch {}
    }
    const rfcPath = join3(dir, "rfc.md");
    if (!existsSync2(rfcPath))
      continue;
    try {
      const content = readFileSync4(rfcPath, "utf8");
...
var STRICT_REQ_ID_RE = /^[A-Z][A-Z0-9]*(?:-[A-Z0-9]+)*-R-\d{3}$/;
var CONCEPT_WHITESPACE_FIELDS = ["title", "summary"];
function schemaErrorsToViolations(errors, nodeId, rawFm) {
  ...
  for (const err of errors) {
    ...
    if (keyword === "not")
      continue;
    if (field === "origin_decision_ref")
      continue;
    if (keyword === "required" && params.missingProperty === "origin_decision_ref")
      continue;
    if (keyword === "oneOf")
      continue;
    if (schemaPath && schemaPath.includes("/oneOf/"))
      continue;
    if (keyword === "required") {
...
function checkRequirementsFile(fileContent, fileAbsPath, targetNodes, rfcMode) {
  ...
  const fileLabel = basename2(fileAbsPath);
  ...
  const isIndividualReqFile = fileAbsPath.includes("/requirements/") && !fileAbsPath.endsWith("/requirements.md") && !fileAbsPath.endsWith("/constraints.md");
  ...
  for (const section of sections) {
    if (rfcMode && targetIds.size > 0 && !targetIds.has(section.id))
      continue;
    const id = section.id;
...
`)) {
    ...
    for (const { filePart, anchor } of extractRelativeLinks(rawChunk)) {
      const targetAbsPath = resolve2(dirname3(fileAbsPath), filePart);
      if (!existsSync2(targetAbsPath)) {
        violations.push({
          nodeId: id,
        ...
        try {
          const targetContent = readFileSync4(targetAbsPath, "utf8");
...
function emitLintDrift(projectDir, rfcUid, nodeId, violation) {
  try {
    const journalPath = join3(projectDir, "hooks", "journal.mjs");
    ...
    spawnSync("node", [
      ...
      "LINT_DRIFT",
      "--msg",
      `spec invariant violation on ${nodeId}: ${violation}`,
      "--data",
      JSON.stringify({ node_id: nodeId, invariant })
...
function checkFmBodyVerificationMismatch(fileContent, fileAbsPath, targetNodes, projectDir) {
  ...
  const fmVerification = fileFm && typeof fileFm.verification === "string" ? fileFm.verification.toLowerCase().trim() : null;
  if (!fmVerification)
    return [];
  const sections = parseRequirementsDocument(fileContent);
  ...
  const results = [];
  for (const section of sections) {
    if (!targetIds.has(section.id.toLowerCase()))
      continue;
    const bodyVerification = section.verification && section.verification !== "unknown" ? section.verification.toLowerCase().trim() : null;
    if (!bodyVerification)
      continue;
    if (fmVerification !== bodyVerification) {
      results.push({
        nodeId: section.id,
        violation: `verification-mismatch: requirement "${section.id}" frontmatter verification=${fmVerification} disagrees with body **Verification**: ${bodyVerification} (${projectDir ? relative3(projectDir, fileAbsPath) : basename2(fileAbsPath)})`
      });
    }
  }
  return results;
...
var rfcIdx = argv.indexOf("--rfc");
var rfcFlagPresent = rfcIdx >= 0;
var rfcUidRaw = rfcFlagPresent ? argv[rfcIdx + 1] : undefined;
var rfcUid = rfcFlagPresent && rfcUidRaw && !rfcUidRaw.startsWith("-") ? rfcUidRaw : null;
var rfcMode = rfcUid != null;
...
function walkReqFiles(dir) {
  ...
  for (const entry of readdirSync3(dir, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      ...
      if (name === "requirements.md" || name === "constraints.md" || basename2(dir) === "requirements") {
        results.push(full);
      }
    }
  }
...
var projectDir = process.env.GROUNDWORK_PROJECT_DIR ?? process.env.CLAUDE_PROJECT_DIR ?? process.cwd();
var specDir = join3(projectDir, "doc", "specs");
var index = loadSpecIndex(projectDir);
if (!index) {
  if (!existsSync2(specDir)) {
...
var allNodes = Object.values(index.nodes ?? {});
var targetNodes = allNodes;
var rfcForJournal = rfcUid ?? "R-unknown";
if (rfcMode) {
...
`);
  ...
  const rfcYamlPath = join3(rfcDir, "rfc.yaml");
  let deltaPaths;
  if (existsSync2(rfcYamlPath)) {
    const rfcYaml = load(readFileSync4(rfcYamlPath, "utf8"));
    deltaPaths = Array.isArray(rfcYaml?.spec_delta) ? rfcYaml.spec_delta.map((item) => item.target).filter(Boolean) : [];
  } else {
    const rfcContent = readFileSync4(join3(rfcDir, "rfc.md"), "utf8");
    deltaPaths = parseSpecDeltaTargets(rfcContent);
  }
  const missingPaths = deltaPaths.filter((t) => !existsSync2(join3(projectDir, t)));
...
`);
  ...
  targetNodes = allNodes.filter((n) => deltaRelPaths.has(n.relPath) || deltaRelPaths.has(n.id) || deltaPaths.includes(n.id));
...
var violations = [];
var diskReqFiles = walkReqFiles(specDir);
for (const absReqPath of diskReqFiles) {
  let raw;
  try {
    raw = readFileSync4(absReqPath, "utf8");
  } catch {
    continue;
  }
...
if (!rfcMode) {
  ...
  const indexedFileCount = new Set(allNodes.filter((n) => n.type === "requirement" && n.relPath).map((n) => n.relPath)).size;
  if (diskCount !== indexedFileCount) {
    const v = `count-parity: ${diskCount} requirement file(s) on disk but ${indexedFileCount} requirement file(s) in index \u2014 run \`node scripts/build-spec-index.mjs\` (or equivalent) to rebuild`;
    violations.push({ nodeId: null, violation: v });
    emitLintDrift(projectDir, rfcForJournal, null, v);
  }
}
...
for (const [relPath, nodes] of byFile) {
  if (!relPath)
    continue;
  const absPath = join3(specDir, relPath);
  if (!existsSync2(absPath))
    continue;
  let fileContent;
  try {
    fileContent = readFileSync4(absPath, "utf8");
  } catch {
    continue;
  }
...
var automatedNodes = targetNodes.filter((n) => n.verification === "automated");
...
var VIEW_ALLOWED_FIELDS = new Set(["type", "id"]);
function levenshtein(a, b) {
  const m = a.length, n = b.length;
  const dp = Array.from({ length: m + 1 }, (_, i2) => [i2, ...Array(n).fill(0)]);
  for (let j = 0;j <= n; j++)
    dp[0][j] = j;
...
var conceptNodes = targetNodes.filter((n) => n.type === "concept");
for (const conceptNode of conceptNodes) {
  const nodeId = conceptNode.id;
  const conceptDir = join3(specDir, dirname3(conceptNode.relPath || ""));
  ...
  for (const view of views) {
    if (!view || typeof view.file !== "string")
      continue;
    const viewAbsPath = join3(conceptDir, view.file);
    if (!existsSync2(viewAbsPath)) {
    ...
    let viewContent;
    try {
      viewContent = readFileSync4(viewAbsPath, "utf8");
    } catch {
      continue;
    }
    const conceptNodeAbsPath = join3(specDir, conceptNode.relPath || "");
    if (viewAbsPath === conceptNodeAbsPath)
      continue;
  ...
  if (views.length > 0 || manifest && Array.isArray(manifest.view_types)) {
    const projectDeclaredNames = new Set(manifest && Array.isArray(manifest.view_types) ? manifest.view_types.filter((vt) => vt && typeof vt.name === "string").map((vt) => vt.name) : []);
...
`);
  ...
  Did you mean "${coreType}"?`;
        ...
        const violation = [
          `unknown-view-type: concept "${nodeId}" declares view type "${view.type}" in views[${i2}]`,
  ...
  Project-declared types: ${[...projectDeclaredNames].join(", ")}.` : ""}`,
          ...
          `  See doc/specs/conventions.md \xA73.`
        ].join(`
`);
        violations.push({ nodeId, violation });
        emitLintDrift(projectDir, rfcForJournal, nodeId, violation);
  ...
  if (manifest) {
    ...
    if (typeNamesConf && Array.isArray(typeNamesConf.names) && typeNamesConf.names.length > 0) {
      if (typeNamesConf.source !== "types") {
        ...
        const scanDir = join3(projectDir, scanRoot);
        if (language === "typescript") {
          for (const name of typeNamesConf.names) {
            const grepResult = spawnSync("grep", ["-rE", `^export (type|interface) ${name}\\b`, "--include=*.ts", scanDir], { encoding: "utf8" });
...
`);
    ...
    if (Array.isArray(opsConf) && opsConf.length > 0) {
      const hooksDir = join3(projectDir, "hooks");
      for (const op of opsConf) {
        const grepResult = spawnSync("grep", ["-rE", `['"]${op}['"]`, "--include=*.mjs", hooksDir], { encoding: "utf8" });
        if (!grepResult.stdout || !grepResult.stdout.trim()) {
    ...
    const readmePath = join3(conceptDir, "README.md");
    if (existsSync2(readmePath)) {
      let readmeContent;
      try {
        readmeContent = readFileSync4(readmePath, "utf8");
      } catch {
        readmeContent = null;
      }
      if (readmeContent) {
        const { data: readmeFm } = parseYamlFrontmatter(readmeContent);
        const specYamlPath = join3(conceptDir, "spec.yaml");
...
if (!rfcMode) {
  const allConceptNodes = allNodes.filter((n) => n.relPath && (n.relPath === "README.md" || n.relPath.endsWith("/README.md") || n.relPath.endsWith("/index.md")));
  const parentFieldAbsent = new Set;
  for (const cn of allConceptNodes) {
    if (!cn.relPath)
      continue;
    const absPath = join3(specDir, cn.relPath);
    let raw;
    try {
...
if (violations.length === 0) {
  process.stdout.write(rfcMode ? `spec lint --rfc ${rfcUid}: clean \u2014 no violations found.
` : `spec lint: clean \u2014 no spec invariant violations found.
`);
  process.exit(0);
}
var lines = violations.map(({ nodeId, violation }) => `LINT_DRIFT ${nodeId}: ${violation}`);
var out = rfcMode ? process.stderr : process.stdout;
