...
6. **Non-trivial work needs a planning artifact first.** Before fanning out `general-purpose` on a feature/SmallRisky task, the orchestrator MUST have a durable `motive_ref` from the `feature-interview` → `planner` pipeline pointing to a motive charter at `.groundwork/motives/<slug>/motive.md`. `feature-interview` is the human front door; `planner` is the delegated stage that emits the charter. Do not embed a memory-only plan and fan out. Trivial / small-clear / docs / obvious-bug paths stay exempt.
