...
export function renderCommitMsgHook({ hooksLibPath, version }) {
  ...
  const nodeLines = [
    ...
    "}",
    "",
    "const houseRulesPath = resolve(libPath, '..', '..', 'plugins', 'house-rules', 'rules', 'commit-message', 'lint.mjs')",
    "if (!existsSync(houseRulesPath)) {",
    "  process.stderr.write(`groundwork commit-msg: WARNING: house-rules plugin not found at ${houseRulesPath} — reinstall groundwork to restore lint\\n`)",
    "  process.exit(0)",
    "}",
    "",
    "let lintMessage",
