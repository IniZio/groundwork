...
- **Verification**: automated — Run each invocation from the repo root with a nonexistent motive slug; schema validation fires before any filesystem write so no state is mutated. Assert exit code 2 and the expected stderr phrase. Source: `hooks/journal.mjs` lines 616–621.
- **Criticality**: must
