...
- **Why** — Unstructured DECISION events cannot be traced back to a specific decision id; a missing `id` makes the decision unaddressable in cross-references, a missing `decision` leaves the outcome undocumented, and a missing `rationale` prevents future reviewers from following the reasoning chain.
...
- **Verification**: manual — Manual inspection — verify that the journal CLI enforces field presence and exits 2 with the missing key named. Confirmed via code inspection of `hooks/journal.mjs` field-validation branch.
- **Criticality**: must
