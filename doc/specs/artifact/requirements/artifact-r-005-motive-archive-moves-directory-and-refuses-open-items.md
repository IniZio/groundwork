---
id: "artifact-r-005"
title: "Motive archive moves directory and refuses open items without --force"
...
status: open
source: "groundwork-development#D-23"
---
...
Archiving a motive with unresolved open items silently buries declared-incomplete work; the guard surfaces the oversight at archive time, and `--force` provides an explicit escape hatch for intentionally deferred items, preventing accidental loss of the open-items signal.
...
Manual demonstration — run `journal motive archive` against a charter with an open TBD; confirm non-zero exit and directory intact. Then re-run with `--force`; confirm directory moved and MILESTONE event present in journal.
