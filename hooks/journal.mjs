 ...
 *   journal graph <slug> [--json|--canvas]
 *   journal help [<cmd>]
 *
...
import { migrateAutoGeneratedTickets } from './lib/motive-tickets.mjs'
import { assembleMotiveGraph } from './lib/motive-graph.mjs'
import { toJsonCanvas } from './lib/motive-canvas.mjs'
...
const HELP = {
  append: {
    ...
    flags: [
      ...
      '  data.status       optional; proposed (default) | accepted | rejected | superseded',
      ...
      '',
      'PAUSE payload (when --type PAUSE): continuation state read back by /resume',
      '  data.pointer      where execution stopped (e.g. slice id, file:line, step name)',
      '  data.summary      what was accomplished up to this point',
      '  data.next_actions array of strings — what to do when resuming',
    ],
  },
  show: {
  ...
  graph: {
    summary: 'assemble a motive latent graph (nodes + edges) as schema-stable JSON',
    usage: 'journal graph <slug> [--json|--canvas]',
    flags: [
      '<slug>     motive identifier (required positional)',
      '--json     emit the D-5 graph doc as JSON to stdout (default behaviour)',
      '--canvas   emit a JSON Canvas document (Obsidian .canvas) to stdout',
      '',
      'Example (vault redirect):',
      '  journal graph <slug> --canvas > /path/to/vault/<slug>.canvas',
    ],
  },
  motive: {
  ...
  'migrate-tickets': {
    summary: 'delete legacy auto-generated ticket files carrying the autogen footer',
    usage: 'journal migrate-tickets <slug>',
    flags: [
      '<slug>  motive slug (required positional)',
      '',
...
async function cmdCompile(args) {
  ...
  if (!toStdout) {
    ...
    regenerateMotiveTraceHtml(projectDir, motive)
  }
...
function cmdMotiveArchive(args) {
  ...
  regenerateMotiveTraceHtml(projectDir, slug)
...
function cmdMigrateTickets(args) {
  const { positionals } = parseFlags(args)
  const slug = positionals[0]
  if (!slug) die('migrate-tickets requires a motive slug', 2)

  const { projectDir } = resolveContext()
  const motiveDir = path.join(projectDir, '.groundwork', 'motives', slug)
  if (!existsSync(motiveDir)) die(`motive directory not found: ${motiveDir}`)

  const deleted = migrateAutoGeneratedTickets(motiveDir)
  if (deleted.length === 0) {
    process.stdout.write('journal: migrate-tickets: nothing to delete\n')
  } else {
    process.stdout.write(`journal: migrate-tickets: deleted ${deleted.length} autogen ticket(s)\n`)
  }
}
...
async function cmdGraph(args) {
  ...
  const slug = positionals[0]
  if (!slug) die('graph requires a motive slug', 2)
  if (flags.json && flags.canvas) die('--json and --canvas are mutually exclusive', 2)

  const { projectDir } = resolveContext()
  const motiveDir = path.join(projectDir, '.groundwork', 'motives', slug)
  if (!existsSync(motiveDir)) die(`motive "${slug}" not found`, 1)

  const graph = await assembleMotiveGraph({ projectDir, slug })
  if (flags.canvas) {
    const canvas = toJsonCanvas(graph)
