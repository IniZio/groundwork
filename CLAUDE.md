...
- Update slice status or fields mid-run: `bin/ledger set <id> --status in_progress|complete [--wave N] [--desc "…"]`; add new slices with `bin/ledger add <id> [--wave N] [--desc "…"] [--blocked-by a,b] [--acceptance "a;b"] [--ticket <tid>] [--covers-ac "AC1,AC2"] [--kind plan|diagnose|design|impl]` (kind defaults to `impl`). `--ticket` links the slice to a ticket document; `--covers-ac` records which acceptance criteria this slice covers (drives `AC_COVERAGE` events on complete). Remove slices with `bin/ledger rm <id>`. Kinds let the ledger represent non-implementation phases (planning, diagnosis, design) as first-class items, making the ledger the whole-session spine rather than implementation-only.
