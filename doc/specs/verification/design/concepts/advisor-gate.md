...
The orchestrator runs `gw ledger gate --motive <slug> advisor APPROVE --token <write_token> --citation <file:line>` after receiving an APPROVE verdict. This writes `gate.advisor = "APPROVE"` into the active run ledger, which the stop gate reads on the next session-end attempt.
