#!/usr/bin/env node
// @bundle-source-hash: 7216000bf96052b2c7a7373eaef0b4b9adc2161645eec950a93a01b1bf8ca9d8
// @bun
...
// hooks/spec.mjs
import { existsSync as existsSync2, readFileSync as readFileSync4, mkdirSync, writeFileSync, readdirSync as readdirSync3 } from "fs";
import { join as join3, dirname as dirname3, basename as basename2, resolve as resolve2 } from "path";
...
function findConceptDir(conceptId, sd) {
  const files = walkSpecFiles(sd);
  for (const { absPath } of files) {
    if (!absPath.endsWith("index.md") && !absPath.endsWith("README.md"))
      continue;
    const { data } = parseYamlFrontmatter(readFileSync2(absPath, "utf8"));
    if (String(data.id) === conceptId)
      return dirname2(absPath);
  }
  return null;
}
...
async function loadSpecManifest(conceptDir) {
...
function randomSuffix(existingIds) {
...
async function runBuild(sd, { silent = false } = {}) {
  ...
  const buildErrors = errors.filter((e) => e.type === "duplicate_id" || e.type === "parent_dir_mismatch" || e.type === "path_in_verify");
...
`);
        ...
        process.stderr.write(`spec: parent/directory mismatch for node "${e.nodeId}"
` + `  frontmatter concept: "${e.frontmatter}"
` + `  directory implies:   "${e.directory}"
` + `  in: ${e.path}
`);
      } else if (e.type === "path_in_verify") {
        process.stderr.write(`spec: path-like token in verify field of "${e.nodeId}": "${e.token}"
...
`);
  ...
  writeFileSync(join3(genDir, "index.json"), JSON.stringify(indexJson, null, 2) + `
`, "utf8");
  const reqs = Object.values(nodes).filter((n) => n.type === "requirement");
  const projectRootDir = dirname3(dirname3(sd));
  ...
  const unverifiedAutomated = reqs.filter((r) => r.verification === "automated" && lookupVerifies(verifiesMap, r.id).length === 0).map((r) => r.id).sort();
  const coverage = {
    total: reqs.length,
  ...
  writeFileSync(join3(genDir, "coverage.json"), JSON.stringify(coverage, null, 2) + `
`, "utf8");
  const allNodes = Object.values(nodes);
  const requirementNodes = allNodes.filter((n) => n.type === "requirement");
  const conceptNodes = allNodes.filter((n) => n.type !== "requirement").sort((a, b) => a.id.localeCompare(b.id));
  const mdLines = [
    "# Spec Index",
    "",
  ...
  ];
  {
    ...
    for (const entry of conceptDirs) {
      ...
      const { manifest, errors: manifestErrors } = await loadSpecManifest(join3(sd, dirName));
      ...
      if (hasManifest) {
        ...
        views = manifest.views && manifest.views.length > 0 ? manifest.views.map((v) => v.type).filter(Boolean).join(", ") : "\u2014";
      } else {
  ...
  for (const concept of conceptNodes) {
    const conceptReqs = requirementNodes.filter((n) => n.concept === concept.id).sort((a, b) => a.id.localeCompare(b.id));
    for (const req of conceptReqs) {
      outputReqIds.add(req.id);
      if (!req.anchor) {
        process.stderr.write(`spec: requirement "${req.id}" has no anchor \u2014 omitted from index.md to avoid a broken link
`);
      }
    }
    const anchoredReqs = conceptReqs.filter((n) => n.anchor);
    if (anchoredReqs.length === 0)
      continue;
  ...
  const uncategorized = requirementNodes.filter((n) => !outputReqIds.has(n.id)).sort((a, b) => a.id.localeCompare(b.id));
...
`);
  ...
  writeFileSync(join3(genDir, "index.md"), mdLines.join(`
`), "utf8");
  if (!silent) {
    process.stdout.write(`spec: built index \u2014 ${Object.keys(nodes).length} nodes, ${reqs.length} requirements
`);
  }
}
...
function cmdInit(_args) {
  ...
  const conceptId = "C-" + projectName.toUpperCase().replace(/[^A-Z0-9]+/g, "-").replace(/^-+|-+$/g, "");
...
`);
  writeFileSync(join3(sd, "index.md"), readme, "utf8");
...
async function cmdBuild(_args) {
  const sd = resolveSpecDir();
  if (!existsSync2(sd))
    die('doc/specs/ not found \u2014 run "spec init" first', 1);
  await runBuild(sd);
}
...
function cmdReqNew(args) {
  ...
  if (!conceptId || !kebabName)
    die("usage: spec req new <concept-id> <kebab-name>", 2);
  ...
  if (!conceptDir)
    die(`concept "${conceptId}" not found in doc/specs/`, 1);
  ...
  for (const { absPath } of allFiles) {
    ...
    if (data.id) {
      ...
      if (m)
        existingSuffixes.add(m[1]);
    }
  }
  const suffix = randomSuffix(existingSuffixes);
  const conceptSuffix = conceptId.replace(/^C-/, "");
  const reqId = `${conceptSuffix}-R-${suffix}`;
  const reqDir = join3(conceptDir, "requirements");
  mkdirSync(reqDir, { recursive: true });
  const reqFile = join3(reqDir, `${kebabName}.md`);
  if (existsSync2(reqFile))
    die(`requirement file already exists: ${reqFile}`, 1);
  const content = [
    "---",
    `id: ${reqId}`,
    `concept: ${conceptId}`,
...
async function cmdShow(args) {
  ...
  if (!idx)
    die('index not found \u2014 run "spec build" first', 1);
  const node = idx.nodes[id];
  if (!node)
    die(`node "${id}" not found in index`, 1);
  const nodeAbsPath = join3(sd, node.relPath);
  if (flags.full) {
    if (!existsSync2(nodeAbsPath))
      die(`file not found: ${node.relPath}`, 1);
    process.stdout.write(readFileSync4(nodeAbsPath, "utf8"));
    return;
  }
  ...
  if (node.concept)
    lines.push(`Concept: ${node.concept}`);
  if (node.ears)
    lines.push(`EARS: ${node.ears.slice(0, 100)}`);
  else if (node.summary)
    lines.push(`Summary: ${node.summary.slice(0, 100)}`);
  if (node.pattern)
    lines.push(`Pattern: ${node.pattern}`);
  if (node.verification)
    lines.push(`Verification: ${node.verification}  Criticality: ${node.criticality || "must"}`);
  if (node.refs && node.refs.length)
    lines.push(`Refs: ${node.refs.slice(0, 5).join(", ")}`);
  const display = lines.slice(0, 8);
  process.stdout.write(display.join(`
`) + `
`);
  process.stdout.write(`\u2500\u2500\u2500 ${display.length} line(s) shown. Full form: ~${tokenCost} tokens  (spec show ${id} --full)
`);
}
async function cmdSearch(args) {
  ...
  if (!query)
    die("usage: spec search <query> [--limit N]", 2);
  ...
  for (const node of Object.values(idx.nodes)) {
    const haystack = [node.id, node.title, node.summary, node.ears, node.relPath].filter(Boolean).join(" ").toLowerCase();
...
async function cmdTree(args) {
  ...
  if (isNaN(depth) || depth < 1)
    die("--depth must be a positive integer", 2);
  const sd = resolveSpecDir();
  if (!existsSync2(sd))
    die('doc/specs/ not found \u2014 run "spec init" first', 1);
  await ensureFreshIndex(sd);
...
`);
    if (currentDepth < depth) {
      ...
      for (const child of children)
        renderTree(child, currentDepth + 1);
...
function cmdDelegate(sub, args) {
  const hooksDir = dirname3(fileURLToPath2(import.meta.url));
  const repoRoot = process.env.CLAUDE_PLUGIN_ROOT ?? resolve2(dirname3(hooksDir), "");
  const bundlePath = resolve2(repoRoot, `dist/hooks-spec-${sub}.mjs`);
  const scriptPath = resolve2(repoRoot, `hooks/spec-${sub}.mjs`);
  const useBundle = existsSync2(bundlePath);
  const useSource = !useBundle && existsSync2(scriptPath);
  if (!useBundle && !useSource) {
    process.stderr.write(`spec: the "${sub}" subcommand requires spec-${sub}.mjs which is not installed.
...
async function main() {
  ...
  try {
    switch (cmd) {
      ...
      case "tree":
        return await cmdTree(rest);
      case "deps":
        return await cmdDeps(rest);
      default:
        if (DELEGATED.has(cmd))
          return cmdDelegate(cmd, rest);
        die(`unknown command "${cmd}". Run spec --help for a list.`, 2);
    }
  } catch (e) {
...
var isMain = process.argv[1] && import.meta.url && process.argv[1].endsWith("spec.mjs");
if (isMain) {
  main().catch((e) => {
