...
- **Verification**: verified — `test/hooks/stop-gate-checkpoint.test.ts` (AC-8: invalid seal with checkpoint_hold blocks fail-closed) and `test/hooks/sealed-gate-vectors.test.ts` (key file deleted → stop-gate blocks); `// @verifies CHECKPOINT-R-005` annotated in both.
- **Criticality**: must
