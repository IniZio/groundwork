...
5. **Advisor gate before done.** Always run the `advisor-gate` completion gate (`[qa if interactive UI] → critic (evidence+quality) → advisor APPROVE`) and record `gate.advisor` in the ledger before declaring a task complete. Trivial work (≤2 files, ≤1 behavior, <1h) goes directly to advisor.
