...
- Record the advisor verdict in the ledger: `gw ledger gate --motive <slug> advisor APPROVE --token <write_token> --citation <file:line>` (add `--rubric …` for the full object form). **This write is mandatory** — the stop-gate reads `gate.advisor` from the ledger; invoking `advisor()` alone does not release the gate.
