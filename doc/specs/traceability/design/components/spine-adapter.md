 ...
 * @property {() => CoverageMap}          getCoverageMap       - .groundwork/spec-build/coverage.json by_requirement map (generated by hooks/spec.mjs)
 */
```
...
| Coverage map | `.groundwork/spec-build/coverage.json` (generated by `hooks/spec.mjs`) |
...
**Decision-mediated** (fallback): when `test_paths` is absent, adapter cross-joins `slice.decisions` against `spec-requirement.origin_decision_ref` in `.groundwork/spec-build/coverage.json`. This is coarse (one-to-many) and labeled as decision-mediated in the graph output.
