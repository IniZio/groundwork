...
- **`pause`** — the write-side complement to `resume`. Records a PAUSE event with `{pointer, summary, next_actions}` into the motive journal; the compiler surfaces it as `agent.last_pause` and folds `next_actions` into `agent.resume.next_actions` for the next session. Use `/pause` at end-of-session; use `/resume` at start-of-session.
...
- Do NOT use `resume` to write continuation state — use `pause` instead (it appends the PAUSE event).
