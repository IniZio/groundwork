...
- **Limitation** — Enforcement covers message shape only: type, scope, length, and body presence. It cannot detect a well-formed subject that describes different work than the commit contains; human review of the commit content against the stated change remains the only check on subject accuracy.
