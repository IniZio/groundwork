...
The adapter cross-joins `slice.decisions` against `requirement.origin_decision_ref` via `.groundwork/spec-build/coverage.json`. This path is coarse (one slice may cover multiple requirements sharing the same decision ref) and labeled as decision-mediated in the graph.
