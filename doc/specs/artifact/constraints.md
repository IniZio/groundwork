...
A groundwork ticket filename **shall** follow the pattern `<NN>-<type>-<slug>.md`, where `<NN>` is a zero-padded two-digit ordinal unique within the motive's ticket corpus, `<type>` is one of the eight values in the controlled type vocabulary (see §Ticket type vocabulary in the data model), and `<slug>` is a kebab-case description. The ticket `Type:` metadata field **shall** carry the same value as the filename type segment. The ticket id is derived from the filename stem and is decoupled from any ledger slice id.

The controlled type vocabulary is a **closed** set of exactly these eight values: `research`, `choose`, `model`, `build`, `grill`, `spec`, `fix`, `chore`. A ticket `type` value outside this set is invalid. The `research` type additionally requires a primary-source citation in the Evidence section (lint enforcement is a separate slice; this requirement records the rule only).

- **Why** — a predictable filename pattern lets any tool enumerate, sort, and categorise the ticket corpus without reading file contents; the ordinal provides stable sort order within a motive; the type prefix makes the nature of work visible in the directory listing. A closed vocabulary prevents type proliferation and enables `--kind` mapping on the ledger CLI; decoupling the ticket id from the slice id lets a ticket span multiple sessions and slices without renaming.
- **Fit criterion** — a ticket file named `01-research-domain-model.md` is valid; a ticket named `1-research-domain-model.md` (missing leading zero) is invalid; a ticket named `01-spike-domain-model.md` (unrecognised type `spike`) is invalid; a ticket whose `Type:` field is `research` but whose filename type segment is `build` is invalid; the ticket id `01-research-domain-model` is independent of any ledger slice id, so renaming a slice does not affect the ticket reference.
- **Verification**: manual — filename pattern validation and type membership are enforced by a directory scan; `Type:` ↔ filename segment agreement is validated by the ticket parser.
- **Criticality**: must · **Source** groundwork-development#D-73, groundwork-development#D-74
