...
Wave/slice count pacing is a proxy for delivery checkpoints. Milestone pacing replaces the proxy with a direct human-verified shippable increment: the gate releases only when a named human has confirmed the named artifacts. This aligns the pacing model with the motive definition of a milestone ("a shippable increment with named artifacts that a human signs off on; pacing gate releases on human verification, not wave count").

## Fit criterion

With `pacing.policy = "milestone"` and `milestone_artifacts` declared but `milestone_signoff` absent (or `verdict: "REJECT"`), `ledger claim` for a slice in a new unit exits 1 and the block message names the outstanding milestone. With `milestone_signoff.verdict = "APPROVE"` present, `ledger claim` exits 0.

## Verification procedure

Automated — covered by tests in S7's test suite (milestone enforcement). S6 (this spec) is design-only; the fit criterion is not testable until S7 lands.
