...
| `self-test` | `test_paths` on a slice, or decision-mediated via `.groundwork/spec-build/coverage.json` | `test/hooks/traceability.test.ts` |
...
**Decision-mediated linkage** (fallback): when `test_paths` is absent, the adapter cross-joins `slice.decisions` against `spec-requirement.origin_decision_ref` values in `.groundwork/spec-build/coverage.json`. This path is coarse (one-to-many) and labeled accordingly in the graph.
