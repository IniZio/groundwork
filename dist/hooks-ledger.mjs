...
function _loadMotiveFold(projectDir, motiveId) {
  if (!motiveId)
    return null;
  const journalDir = path4.join(projectDir, ".groundwork", "journal");
  if (!existsSync3(journalDir))
    return null;
  try {
...
function _loadCharterAcIds(projectDir, motiveId) {
  if (!motiveId || !projectDir)
    return new Set;
  const motivePath = path4.join(projectDir, ".groundwork", "motives", motiveId, "motive.md");
  if (!existsSync3(motivePath))
    return new Set;
  try {
    const content = readFileSync5(motivePath, "utf8");
...
` + `Stale paths: ${hashCheck.staleArtifacts.join(", ")}`);
      ...
      for (const artifact of artifacts2) {
        if (artifact.kind !== "live_url" && artifact.path && !existsSync3(artifact.path)) {
...
function assertResolvableCitation(rawCitation) {
  ...
  while (!citResolved && (refMatch = refPattern.exec(citText)) !== null) {
    ...
    const absFile = path4.isAbsolute(filePart) ? filePart : path4.resolve(process.cwd(), filePart);
    if (existsSync3(absFile)) {
      try {
...
function writeGateArtifact({ runId, which, verdictRaw, value, hasObj }) {
  ...
  const gatesDir = path4.join(base, ".groundwork", "gates");
  try {
    mkdirSync4(gatesDir, { recursive: true });
  } catch {
    return;
  }
  const filename = `${runId ?? "unknown"}.md`;
  const filePath = path4.join(gatesDir, filename);
...
function cmdInit(args) {
  ...
  if (src) {
    let raw;
    try {
      raw = src === "-" ? readFileSync5(0, "utf8") : readFileSync5(src, "utf8");
    } catch (e) {
...
function cmdShow(id) {
  ...
  const acceptance = Array.isArray(s.acceptance) && s.acceptance.length ? s.acceptance.map((a, i) => `    [${i + 1}] ${a}`).join(`
`) : "    (none)";
