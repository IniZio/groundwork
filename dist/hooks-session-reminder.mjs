...
function emitHookEvent(opts = {}) {
  ...
  try {
    if (!VALID_TYPES.includes(type2)) {
      process.stderr.write(`journal: emitHookEvent: invalid type "${type2}" \u2014 event not written
`);
      return { ok: false, error: `invalid type: ${type2}` };
    }
...
function buildSpecSkeleton(projectDir) {
  try {
    ...
    if (!index) {
      const { nodes: nodes2 } = buildIndexData(sd);
      index = { nodes: nodes2 };
    }
...
function activeRunBlock(projectDir, sessionId) {
  ...
  if (!ledger || ledger.active !== true)
    return "";
  if (sessionId && typeof ledger.session_id === "string" && ledger.session_id !== sessionId)
    return "";
  ...
  try {
    ...
    for (const s of slices) {
      if (!s || EXEMPT_KINDS.has(s.kind ?? "impl"))
        continue;
      const w = s.wave ?? 0;
...
var reminder = `# groundwork \u2014 Orchestrator Mode (max fan-out)
...
Before a wave, state: **Agent** (which specialist) \xB7 **Reason** (why this one) \xB7 **Success criteria**. This surfaces bad routing before tokens are spent.
...
if (sessionId || transcriptPath) {
  ...
  if (transcriptPath)
    lines.push(`- transcript_path: ${transcriptPath}`);
...
var projectDir = typeof input?.cwd === "string" && input.cwd || process.env.CLAUDE_PROJECT_DIR || process.cwd();
try {
