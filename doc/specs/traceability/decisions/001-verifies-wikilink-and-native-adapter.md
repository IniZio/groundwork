...
`NativeSpineAdapter` (defined in `hooks/lib/traceability-adapter.mjs`) is the default `SpineAdapter`. It reads from groundwork's native data sources: run ledger, journal events, `.groundwork/spec-build/coverage.json` (generated by `hooks/spec.mjs`), and motive.md.
...
- `verifies-scan.mjs` must be run as part of coverage reporting; its output feeds `.groundwork/spec-build/coverage.json` (via `hooks/spec.mjs`).
