...
When the orchestrator runs `gw ledger fog <id> --desc "…" --question "…"`, the `gw ledger` command surface **shall** create a slice with `kind: "fog"` and no acceptance criteria, and `gw ledger frontier` **shall** exclude all slices with `kind: "fog"` from its output.
