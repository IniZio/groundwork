...
If the Stop hook fires and any compiled DECISION entry for the current motive carries an empty `alternatives` list (`[]`) or is flagged `unmarked_collision: true`, then the Stop hook **shall** append a non-blocking advisory message naming the ids of the offending DECISION events; the advisory **shall not** block session end.

- **Why** — a DECISION event whose `alternatives` array is empty was recorded without documenting what other options were considered, which reduces the decision's future reviewability; an `unmarked_collision` flag signals that two events share the same id without an explicit `data.revises` marker, indicating a likely copy-paste error or accidental id reuse. Surfacing both gaps at session-end as non-blocking advisories gives the orchestrator the option to correct them before closing without preventing completion of sessions where either condition is intentional or deferred.
- **Fit criterion** — with a DECISION event carrying `alternatives: []` and another compiled entry carrying `unmarked_collision: true`, the Stop hook output contains advisory lines naming each offending id and the session is permitted to end (the gate is not blocked); when all compiled DECISION entries carry non-empty `alternatives` and no `unmarked_collision` flag, no advisory of this type is emitted.
- **Verification** manual · **Criticality** should · **Source** groundwork-development#D-59
