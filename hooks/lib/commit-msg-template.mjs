...
export function renderCommitMsgHook({ hooksLibPath, version }) {
  ...
  const bashLines = [
    ...
    "  _GW_RUNTIME=node",
    "else",
    "  printf 'groundwork commit-msg: no bun/node, skipping lint\\n' >&2",
    "  exit 0",
    "fi",
    "",
    "_GW_TMP=\"$(mktemp /tmp/gw-commit-msg-XXXXXX.mjs)\"",
    "cat > \"$_GW_TMP\" <<'GWEOF'",
    ...nodeLines,
    "GWEOF",
    "COMMIT_MSG_FILE=\"$1\" GROUNDWORK_HOOKS_LIB=\"$GROUNDWORK_HOOKS_LIB\" COMMENT_CHAR=\"$COMMENT_CHAR\" CLEANUP_MODE=\"$CLEANUP_MODE\" REPO_ROOT=\"$REPO_ROOT\" \"$_GW_RUNTIME\" \"$_GW_TMP\"",
    "_GW_STATUS=$?",
    "rm -f \"$_GW_TMP\"",
    "exit $_GW_STATUS",
    "",
