...
A chain with unclassified edges is indistinguishable from one with proven edges. The classification must be visible so a reviewer can immediately identify which links need attention without reading the underlying data.

## Fit criterion

In a session with a manual review of the rendered output: at least one edge is labeled/styled as proven (advisor APPROVE present), at least one edge is labeled/styled as unproven (slice with no gate), and the visual distinction between the two states is unambiguous to a reviewer unfamiliar with the data.

## Verification procedure

Manual inspection of the rendered traceability view:
